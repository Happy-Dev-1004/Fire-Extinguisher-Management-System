...
export interface Entrada {
  telefone_normalizado: string;
  // The phone to SEND replies to. Z-API needs the original form (with country
  // code) — the normalized key drops it. Defaults to telefone_normalizado when
  // omitted (tests). The webhook always passes the raw phone.
...
// Sends the question for the current etapa (or the photo prompt). `envio` is the
// phone to send to (raw, with country code).
async function perguntar(deps: Deps, envio: string, etapa: number): Promise<void> {
  const p = pergunta(etapa);
  if (!p) return;
  await deps.enviar(envio, p.pergunta);
}
...
export async function processarRdo(deps: Deps, e: Entrada): Promise<boolean> {
  const tel = e.telefone_normalizado;        // session/store key
  const envio = e.telefone_envio ?? tel;     // phone to send replies to (raw)
  ...
  if (CANCELAR.includes(lower)) {
    ...
    await deps.enviar(envio, "❌ RDO cancelado. Envie *RDO* quando quiser começar de novo.");
    return true;
  }
  if (VOLTAR.includes(lower) && !sessao.aguardando_fotos) {
    ...
    await deps.enviar(envio, "↩️ Voltando à pergunta anterior.");
    await perguntar(deps, envio, novaEtapa);
    return true;
  }
  ...
  if (!p) {
    ...
    await finalizar(deps, tel, envio, sessao.rdo_id);
    return true;
  }
  ...
  if (p.tipo === "fotos") {
    ...
    if (e.imageUrl) {
      ...
      await deps.enviar(envio, "📷 Foto recebida. Envie mais ou escreva *pronto* para concluir.");
      return true;
    }
    ...
    await deps.enviar(envio, "Envie as *fotos* do dia, ou escreva *pronto* para concluir.");
    await deps.atualizarSessao(tel, { ultima_msg_id: e.messageId ?? null });
    return true;
  }
  ...
  if (!texto) {
    ...
    await deps.enviar(envio, "Por favor, responda à pergunta com uma mensagem de texto.");
    return true;
  }

  const v = validarResposta(p, texto);
  if (!v.ok) {
    await deps.enviar(envio, `⚠️ ${v.erro}`);
  ...
  await perguntar(deps, envio, proxima);
  return true;
}

async function finalizar(deps: Deps, tel: string, envio: string, rdoId: string): Promise<void> {
  const row = await deps.finalizarRdo(rdoId);
  await deps.apagarSessao(tel);
  await deps.enviar(envio, montarResumo(row));
}
